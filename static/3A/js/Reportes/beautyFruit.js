...
selector_pais.addEventListener("change", (event) => {
    changeClean();
});
...
const choicePais = new Choices('#selector_pais', {
    allowHTML: true,
    shouldSort: false,
    placeholderValue: 'SELECCIONE PAÍS',
    searchPlaceholderValue: 'Escriba para buscar..',
    itemSelectText: ''
});
...
const dataInicial = async () => {
    ...
    try {
        ...
        if (data.Message === "Success") {
            ...
            let result1 = [];
            result1.push();
            data.Pais.forEach((datos) => {
                result1.push({
                    value: datos.IdPais, label: datos.Descripcion
                });
            });
            choicePais.setChoices(result1, 'value', 'label', true);
        } else {
            var nota = data.Nota
            var color = "red";
...
const dataDateTableTamaños = async () => {
    ...
    try {
        ...
        formData.append("IdPais", getValuePais());
...
const descargarArchivo = async (name,tipo) => {
    openLoading();
    try {
        const formData = new FormData();
        formData.append("Incio", desde.value);
...
const dataDateTableDynamic = async () => {
    try {
        ...
        jsonDatos.forEach(record => {
            if (record["_28_55"] !== "0.00") {
            ...
            if (record["_60_100"] !== "0.00") {
                datosProcesados.push({
                    ...record,
                    Calibre: "60-100",
                    Valor: record["_60_100"]
                });
            }
            
            if (record["_110_125"] !== "0.00") {
                datosProcesados.push({
                    ...record,
                    Calibre: "110-125",
                    Valor: record["_110_125"]
                });
            }
            
            if (record["_135_198"] !== "0.00") {
                datosProcesados.push({
                    ...record,
                    Calibre: "135-198",
                    Valor: record["_135_198"]
                });
            }
        });

        $("#pivot-table").pivotUI(datosProcesados, {
            rows: ["Variedad", "Marca", "Calibre"], 
            cols: ["Semana", "Pais"],  
            vals: ["Valor"], 
            aggregatorName: "Sum",
            rendererName: "Table",
...
function getValuePais() {
    return choicePais.getValue() ? choicePais.getValue().value : '';
}
...
function innerPopAll() {
    ...
    all_content.innerHTML = `
        ...
        <div style="text-align: right; margin-top: 10px;">
            ...
            <a onclick="descargarArchivo('Promedio_por_pais_','DP');" style="display: block;">
