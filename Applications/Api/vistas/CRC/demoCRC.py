                ...
                with connections['S3A'].cursor() as cursor:
                    sql = f""" 
                        ...
                        SELECT  DLC.Mercado AS MERCADO, ISNULL(DLC.NombreEmbarque, '') AS VAPOR, DLC.PaisDestino AS DESTINO, DLC.IdCliente AS ID_CLIENTE, CONVERT(VARCHAR(30), DLC.Cliente) AS CLIENTE, DLC.Fecha 
                                ...
                                DATEPART(SECOND, DLC.ALTA_REMITO) AS SEGUNDOS, COALESCE(PCR.CANTIDAD,0) AS CANTIDAD, COALESCE(CONVERT(VARCHAR, PCR.CRC),'0') AS CRC, PCR.CALIBRE AS CALIBRE, DLC.Moneda AS TIPO_MONEDA, DLC.FUNCION AS FUNC, DLC.Fecha AS FECHA,
                                YEAR(GETDATE()) AS AÑO
